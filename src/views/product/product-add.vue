  ...
  <div class="common-list-page product_add">
    ...
    <el-tabs v-model="activeName2" style="padding:3px 0 0;background:#fff;">
      ...
      <el-tab-pane label="费用配置" name="second">
        <div class="title"><h3>产品费用信息</h3>
        </div>
        <div class="cost-cont">
          <div class="cost_type_title"><h4>利息：(还款时扣除-后收)</h4>
          </div>
          <el-form :model="dynamicValidateForm" ref="dynamicValidateForm" :rules="dataRule2" label-width="100px" class="demo-dynamic">
            ...
            <el-row>
              ...
              <el-col :span="8">
                <el-form-item label="基准利率类型: ">
                  <span>年化比率</span>
                </el-form-item>
              </el-col>
              <el-col :span="16">
                <el-col :span="8" style="padding:0">
                  <el-form-item label="浮动比率: " prop="rateInterval">
            ...
            <el-row v-for="(domain) in dynamicValidateForm.domains" :key="domain.id">
              <el-col :span="8">
                <el-form-item label="费用名称: ">
                  <span>特定利息</span>
                </el-form-item>
              ...
              <!-- <el-col :span="16">                                      
                                                      <el-col :span="8" style="padding:0">
                                                        ...
                                                        :prop="'domains.' + index + '.value' " :rules="[{ validator: formvalid.isInteger,  trigger: 'blur' },{ required: true, message: '', trigger: 'blur' }]"
                                                          >
                                                            <el-input v-model.trim="domain.value" minlength="1" maxlength="50" placeholder="费用比率" />
                                                          </el-form-item>
                                                        </el-col>
                                                          <el-col :span="1" class='line' style="text-align:center;line-height:40px">
                                                    %
                                                        </el-col>                                                
                                                </el-col> -->
              <el-col :span="16">
                <el-col :span="8" style="padding:0">
                  <el-form-item label="浮动比率: ">
                    <el-input placeholder="费用比率" v-model.number="domain.minvalue" style="width: 100%;" :readonly="true" />
                  </el-form-item>
                </el-col>
                <el-col class="line" :span="1" style="padding:0 ;text-align:center;line-height:40px">%</el-col>
                <el-col :span="8" style="padding:0" class="tlft">
                  <el-form-item label="~">
                    <el-input placeholder="费用比率" v-model.number="domain.maxvalue" style="width: 100%;" :readonly="true" />
                  </el-form-item>
                </el-col>
                <el-col class="line" :span="1" style="padding:0;line-height:40px;margin-left:-50px;">%</el-col>
            ...
            <div class="cost_type_title"><h4>服务费：(放款时-扣除)</h4>
            </div>
            <el-row>
              <el-col :span="8">
                <el-form-item label="费用名称: ">
      ...
      <el-tab-pane label="合同模板" name="three">
        <div class="title"><h3><span style="color:#f56c6c;font-size:12px;"></span>合同模板选择</h3>
        </div>
        <div class="cost-cont">
          <el-table :data="dataList" highlight-current-row @selection-change="handleSelectionChange">
            <el-table-column type="selection" :reserve-selection="true" width="55">
            </el-table-column>
            <el-table-column prop="templateNo" label="合同编号" align="center" />
            <el-table-column prop="contractName" label="合同名称" align="center" />
            <el-table-column prop="contractVersion" label="合同版本" align="center" />
            <el-table-column prop="type" label="合同类型" align="center" min-width="120" :formatter="temptype" />
          </el-table>
        </div>
      </el-tab-pane>
    </el-tabs>
    <el-dialog :title="pricetitle" :visible.sync="showAddDialog" :close-on-click-modal="false" :show-close='false'>
      <el-form :model="fromData" ref="from">
        <el-table :data="fromData.domains" style="width: 100%">
          <el-table-column type="index" label="编号" width="50" />
          <el-table-column prop="compCreidtLevel" label="客户信用等级">
          </el-table-column>
          <el-table-column prop="compRateLevel" label="定价等级">
          </el-table-column>
          <el-table-column label="浮动比率">
...
export default {
  data() {
    ...
    return {
      ...
      dataForm2: {
        compCreidtLevelist: [{ levelName: '' }],
      },
      multipleSelection: [],
      lvInterestarr: [], //产品费用配置数据
  ...
  methods: {
    ...
    openinfo(val) { ///打开详细定价等级
      ...
      _.cloneDeep(this.lvInterestarr).map(el => {
    ...
    lvInterest() {
      ...
      then((res) => {
        loading.close()
        if (res.data.code == '0') {
          this.lvInterestarr = res.data.message.detail
          api.searchCreditLevelist().
          then((res) => {
    ...
    handleAddUpdate() {
      ...
      //     })
      // }
      // console.log(this.lvInterestarr, 7777)
        ...
        if (valid) {
          ...
          this.$refs["dynamicValidateForm"].validate(valid2 => {
            ...
            if (valid2) {
              ...
              // this.dynamicValidateForm.domains.map(el=>{    //改版前的逻辑
              //     el.costType=1;
              //     el.costValueType=2;
              //     el.interestType=1
              //     el.compCreidtLevel=el.levelName
              //     el.cost=el.value
              //     delete el.id
              //     params.costs.push(el)
              //   })
              this.lvInterestarr.map(el => { //改版后的逻辑
                el.costType = 1;
                el.costValueType = 2;
                el.interestType = 1
                delete el.id
