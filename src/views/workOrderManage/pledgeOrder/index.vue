      ...
      <data-list :count-undo="total" ref="dataList" @data-refresh="getData" :filters="filters" :total="total" :toolFilterReceive="true" :tabOptions="tabOptions" @tabclick='tabclick' :routeName="$route.name">
        <div style="width: 100%;padding:0 24px;background:#fff">
          ...
          <el-table :data="dataList" key="2" v-if="showtab==2">
            ...
            <el-table-column prop="userRealname" align="center" width="200" label="执行人">
            </el-table-column>
          </el-table>
          <el-table :data="dataList" key="3" v-if="showtab==3">
            ...
            <el-table-column prop="createTime" label="工单生成时间" align="center" />
            <el-table-column prop="startTime" label="工单开始时间" align="center" />
            <el-table-column prop="endTime" label="工单完成时间" align="center" />
            <el-table-column prop="userRealname" align="center" width="200" label="执行人">
            </el-table-column>
            <el-table-column align="center" width="200" label="操作">
              <template slot-scope="scope">
...
export default {
  ...
  data() {
    return {
      ...
      positionPath: [{ name: '工单管理' }, { name: '抵质押代办工单' }],
      showtab: 0,
      filters: [{
        label: "客户名称：",
        type: "input",
  ...
  methods: {
    tabclick(val) {
      this.showtab = val;
      this.loading = true
      api.getPledgeOrderList({ pageSize: this.$store.state.pagesizes, pageNum: 1, status: val }).
      then((res) => {
        if (res.data.code == '0') {
    ...
    gotoInfo(row) {
      sessionStorage.setItem('pledgeOrderInfo', JSON.stringify(row))
      this.$router.push({ name: '抵质押代办工单详情', query: { id: row.id } })
    },
    openAssignment(row) {
      this.formData = row
      this.worker = ''
    ...
    submit() {
      ...
      this.loading = true
      api.assignmentUserPledge(data).then(({ data }) => {
        this.loading = false
        if (data.code == '0') {
