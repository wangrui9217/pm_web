...
export default {
  ...
  data() {
    return {
      ...
      positionPath: [{ name: '工单管理' }, { name: '企业央行征信录入工单' }],
      showtab: 0,
      filters: [{
        label: "客户名称：",
        type: "input",
  ...
  methods: {
    tabclick(val) {
      this.showtab = val;
      this.loading = true
      api.getOrderListBciApply({ pageSize: this.$store.state.pagesizes, pageNum: 1, status: val }).
      then((res) => {
        if (res.data.code == '0') {
    ...
    getData(params) {
      ...
      api.getOrderListBciApply(params).
      then((res) => {
        if (res.data.code == '0') {
    ...
    gotoInfo(row, type) {
      ...
      this.$router.push({ name: '企业央行征信录入工单详情', query: { id: row.id } })
    },
  },
  created() {
