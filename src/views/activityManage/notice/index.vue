      ...
      <data-list ref="dataList" @data-refresh="getData" @stop-useing="stopUseing" @start-useing="startUseing" @to-add-activity="toAddActivity" :toolFilterReceive="true" :filters="filters" :addActivity="true" :total="total">
        <div style="width: 100%;padding:0 24px;background:#fff">
          <el-table ref="multipleTable" :data="dataList" @selection-change="handleSelectionChange" row-key="id">
            ...
            <el-table-column prop="updateName" label="更新人" width="150" align="center" />
...
export default {
  data() {
    return {
      loading: false,
      positionPath: [{ name: '公告管理' }, { name: '公告列表' }], //路径信息
      filters: [{
          label: "启用状态：",
          type: "select",
          field: "status",
  ...
  methods: {
    getData(params) {
      ...
      api.getEventList(data).then((res) => {
        if (res.data.code == 0) {
          let dataList = res.data.message.list
    ...
    stopUseing() {
      if (this.pidsStatus.length == 0) {
        //没有数据
        this.$message.warning("请选择需要停用的公告！")
        return;
      }
      var params = {
        ids: this.pids,
    ...
    startUseing() {
      if (this.pidsStatus.length == 0) {
        //没有数据
        this.$message.warning("请选择需要启动的公告！")
        return;
      }
      var params = {
        ids: this.pids,
    ...
    dealwith(row) {
      ...
      this.$router.push({ path: '/edit-notice'})
    }
