...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 移除子节点
      removeChildNode (data, parantId) {
        var parantIds = isArray(parantId) ? parantId : [parantId]
        if (parantId.length <= 0) {
          return data
        }
        var ids = []
        for (var i = 0; i < data.length; i++) {
          if (parantIds.indexOf(data[i][this.parentKey]) !== -1 && parantIds.indexOf(data[i][this.treeKey]) === -1) {
