...
<template>
    <div :style="{width:tableWidth}" class='autoTbale'>
        <table class="table table-bordered" id='hl-tree-table'>
            ...
            <tbody>
                <tr @mouseover="ischoose=index" v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent,'active':index==ischoose}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        ...
                        <div v-if="column.type === 'action'">
                          <el-button type="primary"  size="small"  @click="RowClick(item)">修改</el-button>
                          <el-button type="primary" :class="{'el-button--success':item.status==0,'el-button--danger':item.status==1}" size="small"  @click="PassClick(item)">{{item.status=='1'?'停用':'启用'}}</el-button>
                        ...
                        <label @click="toggle(index,item)" v-if="!column.type">
                            ...
                            <!--       <i v-if="item.children&&item.children.length>0" class="ivu-icon" :class="{'ivu-icon-plus-circled':!item.expanded,'ivu-icon-minus-circled':item.expanded }"></i>
                                <i v-else class="ms-tree-space"></i> -->
                                  ...
                                  <i v-if="item.children&&item.children.length>0" class="zk-table--tree-icon zk-icon " :class="{'zk-icon-plus-square-o':!item.expanded,'zk-icon-minus-square-o':item.expanded }"></i>
                                <i v-else class="ms-tree-space"></i>
    ...
    methods: {
      ...
      iconRow() {
        ...
        RowClick(data, event, index, text) {
            ...
            this.$emit('on-row-click', JSON.stringify(result), event, index, text)
        },
        //处理停用
        PassClick(data){
            let result = this.makeData(data)
            this.$emit('on-pass-click', JSON.stringify(result))
        },
        // 点击事件 返回数据处理
        makeData(data) {
